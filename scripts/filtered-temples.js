...
newlink.addEventListener("click", () => {
  let newtemple = temples.filter(
    (temples) => new Date(temples.dedicated).getFullYear() > 2000
  );
  createTempleCard(newtemple, "New Temples");
});

oldlink.addEventListener("click", () => {
  let oldtemple = temples.filter(
    (temples) => new Date(temples.dedicated).getFullYear() < 1990
  );
